...
class IVLoss(Module):
    ...
    def forward(self, y_pred,y,lbl_std):
        """
        
        """
        m = y.shape[0]
        l = torch.matmul(torch.sub(y_pred,y).t(), torch.sub(y_pred,y)*(1/( (lbl_std)**2 + self.epsilon  ) ))
        l = (l/m)
        if self.avg_batch:
            return l/(torch.sum(1/( (lbl_std)**2 + self.epsilon  ) ))
        else:
            return l
