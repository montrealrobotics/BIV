...
# Change Wandb log file from: test      to: estimation_baseline
###################################################################################################
...
class UTKface(Dataset):
    ...
    def __init__(self, path, train = True, transform = None, noise = False, uniform_limits = None):
        self.train = train
        self.images_path = glob.glob(path)
        self.transform = transform
        self.noise = noise
        self.uniform_limits = uniform_limits
    ...
    def unif_label_noise(self, a,b):
        if a<0 or b<0:
            return "Operation can not be done."
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.noise:
            lbl_noise, noise_variance = self.unif_label_noise(self.uniform_limits[0], self.uniform_limits[1])
        else:
